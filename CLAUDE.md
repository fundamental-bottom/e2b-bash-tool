...
- `e2b.ts` - Adapter wrapping @e2b/code-interpreter instances
...
1. **Optional peer dependencies**: just-bash and @e2b/code-interpreter are optional. Dynamic imports with helpful error messages when not installed.
...
4. **Sandbox auto-detection**: The `sandbox` parameter auto-detects both `@e2b/code-interpreter` instances (checks for `sandboxId`, `commands`, `files`, and `kill`) and just-bash `Bash` instances (checks for `exec` method). Users can also provide a custom `Sandbox` implementation.
